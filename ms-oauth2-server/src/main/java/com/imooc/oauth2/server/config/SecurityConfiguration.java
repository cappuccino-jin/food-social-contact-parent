...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    // RedisTokenStore 用于将 token 存储至 Redis
    @Bean
    public RedisTokenStore redisTokenStroe(){
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable()
                .authorizeRequests()
                // 放行的请求
                .antMatchers("/oauth/**", "/actuator/**").permitAll()  // /oauth/**=检查 /actuator/**=健康检查
                .and()
                .authorizeRequests()
                // 其他请求必须认证才能访问
