    ...
    public ResultInfo doSeckill(Integer voucherId, String accessToken, String path) {
        ...
        try {
            ...
            if (isLocked) {
                ...
                // 采用 Redis
                // 扣库存
                // 由于Hash类型的自增是按照步长自增的，所以使用increment 填充-1即可自减
                //long count = redisTemplate.opsForHash().increment(key, "amount", -1);
                //AssertUtil.isTrue(count < 0, "该券已经卖完了");

                // 采用 Redis + Lua 解决问题
                // 扣库存
                Long amount = (Long) redisTemplate.execute(defaultRedisScript, Arrays.asList(key, "amount"));
                AssertUtil.isTrue(amount == null || amount < 1, "该券已卖完了");
            }
        } catch (Exception e) {
            // 手动回滚事务
